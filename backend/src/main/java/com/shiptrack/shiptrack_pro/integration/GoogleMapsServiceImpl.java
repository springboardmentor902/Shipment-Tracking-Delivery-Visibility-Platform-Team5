 ...
 *
 * Compatible with Spring Boot 4.x / Spring Framework 7.x.
 */
@Service
public class GoogleMapsServiceImpl implements GoogleMapsService {

    private static final Logger log =
            LoggerFactory.getLogger(GoogleMapsServiceImpl.class);
    ...
    public Optional<GeoPoint> geocode(String address) {
        ...
        try {
            ...
            if (!"OK".equals(root.path("status").asText())) {
                ...
                log.warn(
                        "Geocoding API returned status {} for address '{}'",
    ...
    @Override
    public Optional<RouteEstimate> getDirections(
            GeoPoint origin,
        ...
        try {
            ...
            return Optional.of(
                    new RouteEstimate(distanceKm, minutes)
            );

        } catch (Exception e) {

            log.warn(
                    "Directions lookup failed: {}",
                    e.getMessage()
            );
    ...
    @Override
    public Optional<RouteEstimate> estimateRoute(
    ...
    @Override
    public List<RouteAlternativeDTO> getRouteAlternatives(
            GeoPoint origin,
            GeoPoint destination) {
        ...
        try {
            ...
            if (!"OK".equals(root.path("status").asText())) {
                ...
                log.warn(
                        "Directions API (alternatives) returned status {}",
                        root.path("status").asText()
                );
            ...
            for (JsonNode routeNode : root.path("routes")) {
                ...
                int minutes =
                        Math.round(seconds / 60.0f);

                JsonNode trafficNode =
                        leg.path("duration_in_traffic");

                Integer trafficMinutes =
                        trafficNode.isMissingNode()
                                ? null
                                : Math.round(
                                        trafficNode
                                                .path("value")
                ...
                String summary =
                        routeNode.path("summary").asText(null);

                alternatives.add(
                        new RouteAlternativeDTO(
                                distanceKm,
                                minutes,
                                trafficMinutes,
                                summary
            ...
            log.warn(
                    "Route alternatives lookup failed: {}",
                    e.getMessage()
            );
    ...
    @Override
    public List<RouteAlternativeDTO> estimateRouteAlternatives(
            String originAddress,
            String destinationAddress) {
        ...
        try {
            ...
            log.warn(
                    "Route alternatives estimation failed for '{}' -> '{}': {}",
                    originAddress,
                    destinationAddress,
                    e.getMessage()
            );
    ...
    private boolean isConfigured() {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            ...
            log.debug(
                    ...
                    "(GOOGLE_MAPS_API_KEY env var unset) - skipping call."
            );
