...
/**
 * registerUser: rejects duplicate emails, validates the role against the Role enum,
 * blocks ADMINISTRATOR at registration (only the AdminSeeder can create that one account),
 * hashes the password with BCrypt, defaults status to ACTIVE.
 *
 * loginUser: verifies credentials (401 on any mismatch, same message either way so no
 * information leaks), blocks non-ACTIVE accounts (403), stamps last_login_at, issues a JWT.
 */
@Service
...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse updateUserRole(Long userId, String newRole) {
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Invalid role: " + newRole + ". Must be one of: " + Arrays.toString(Role.values()));
        }
